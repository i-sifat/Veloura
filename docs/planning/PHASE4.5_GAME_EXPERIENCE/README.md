...
- a signature hero interaction per game (thrown dice, card fan, spinning wheel, swipe stack, pulse ring)
- a hard cognitive-load contract that keeps play surfaces clean

## 2. What this phase is NOT

- **Not new content.** The packs from Phase 3 and 4 are reused as-is. No sub-phase re-seeds content.
  Where a new UI needs grouping (for example three intensity decks), it *derives* that grouping from
  the existing `Difficulty` enum and categories. See D-9 in the master plan.
...
| 4.5.5 | `4.5.5_CREATIVE_CONNECTIONS.md` — swipe stack over the existing conversation pack | 4.5.1 + Phase 3.3 |
| 4.5.6 | `4.5.6_FOLLOW_THE_TEMPO.md` — new pulse-ring game (the only new game) | 4.5.1 |
...
After it merges, 4.5.2 through 4.5.7 are independent of each other and can be built in any order or
split across agents. There is deliberately **no "polish" sub-phase** — those items are folded into the
master plan's existing Phases 6, 7 and 9 via `AMENDMENTS_TO_LATER_PHASES.md`, so nothing is duplicated.
...
| Creative Connections | `/games/creative-connections` | `lib/features/conversation` |
...
Position Library stays out of the grid — it remains feature-flagged off for v1 per D-3.
