...
Runs after Phases 2–7 are merged. This is where the app stops looking good in screenshots and starts feeling premium in the hand.
...
- Gated surfaces: Passionate Roleplay scenes 3–5 · Superhot deck · Superhot Roulette prompts ·
  custom dice faces. Everything else is free.
...
- Verify every phase's `disableAnimations` path: dice at rest, wheel unrotated, cards crossfaded,
  ring colour-pulsing, beats crossfading.
- All icon-only buttons have `Semantics` labels; all tap targets ≥ 44x44.
...
- If shipped: dice contact click, card flip whoosh, wheel tick, tempo metronome. All ≤ 200ms, all
  respecting the Sound pref and the OS silent switch.
...
- Golden tests: Games hub · dice at t = 0.30/0.60/1.00 · card back and front per deck · wheel at
  rest and mid-spin · scene card. Run at 390x844 and 360x800.
...
- Shader warm-up: verify no jank on the first dice throw and first wheel spin after a cold start.

## 7. Documentation

- Update `README.md` (games list + screenshots) and `ARCHITECTURE.md` (new `session`, `games_hub`,
  `follow_the_tempo` modules).
...
- [ ] Reduce-motion and textScale 1.3 verified on all six games.
- [ ] Unified history renders every game's events in Profile → Activity.
- [ ] Golden and copy-rule tests green in CI.
- [ ] Performance budget met on all six games.
